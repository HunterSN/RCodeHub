...
setInfFKNN = setInfF
knn_roc = roc(setInfFKNN$type, as.numeric(KNNwh),levels = c('1', '0'),direction = "<")
plot(knn_roc)
knn_roc$auc
coords(knn_roc, "best")
...
pred = lapply(1:v,function(i, setInf){
  omit = which(grps == i)
  pcl = predict(svm(type ~ OB + DisRwr + HNgoSim + Jaccard + Man, data = setInf[-omit,],  type = 'C-classification', kernel = 'radial'),newdata = setInf[omit,])
...
pred = lapply(1:v,function(i, setInf){
  omit = which(grps == i)
  mol_GBRTF = gbm(type ~ OB + DisRwr + HNgoSim + Jaccard + Man,
...
pred = lapply(1:v,function(i, setInf){
  omit = which(grps == i)
  pcl = predict(NaiveBayes(type ~ OB + DisRwr + HNgoSim + Jaccard + Man,setInf[-omit,]),newdata = setInf[omit,])
...
pred = lapply(1:v,function(i, setInf){
  omit = which(grps == i)
  pcl = predict.glm(glm(type ~ OB + DisRwr + HNgoSim + Jaccard + Man,setInf[-omit,],family=Manomial(link="logit")),newdata = setInf[omit,])
},setInf)
#整合测试结果
LGwh = unlist(pred)
#grps 顺序已经被打乱，重新从小到大排序
...
LG_roc = roc(setInfFLG$type, as.numeric(LGwh),levels = c('0', '1'),direction = "<")
...
write.csv(setInfFKNN,"HNKNNML.csv",quote = F,row.names = F,fileEncoding = 'GBK')


save.image(file = "HNML.RData")

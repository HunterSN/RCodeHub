...
defSet = function(a){
  ...
  setInf[which(setInf$Man >= as.numeric(quantile(setInf$Man)[cutN])),]$BMan = 1
...
pred = lapply(1:v,function(i, setInf){
  omit = which(grps == i)
  pcl = fitted(kknn(type ~ OB + DisRwr + goSim + Jaccard + Man, setInf[-omit,], setInf[omit,], k = KS ,distance = 2 ))
...
pred = lapply(1:v,function(i, setInf){
  omit = which(grps == i)
  pcl = predict(svm(type ~ OB + DisRwr + goSim + Jaccard + Man, data = setInf[-omit,],  type = 'C-classification', kernel = 'radial'),newdata = setInf[omit,])
...
#grps 顺序已经被打乱，重新从小到大排序
setInf$grps = grps
knnT = as.data.frame(table(SVMwh))
setInfF = setInf[order(setInf$grps),]

setInfFSVM = setInfF

#混淆矩阵
MLSVM= table(setInfFSVM$type, SVMwh, dnn =c("真实值","预测值"))
...
pred = lapply(1:v,function(i, setInf){
  omit = which(grps == i)
  mol_GBRTF = gbm(type ~ OB + DisRwr + goSim + Jaccard + Man,
...
#grps 顺序已经被打乱，重新从小到大排序
setInf$grps = grps
knnT = as.data.frame(table(GBRTwh))
setInfF = setInf[order(setInf$grps),]

setInfFGBRT = setInfF

#混淆矩阵
MLGBRT= table(setInfFGBRT$type, GBRTwh, dnn =c("真实值","预测值"))
...
pred = lapply(1:v,function(i, setInf){
  omit = which(grps == i)
  pcl = predict(NaiveBayes(type ~ OB + DisRwr + goSim + Jaccard + Man,setInf[-omit,]),newdata = setInf[omit,])
...
#grps 顺序已经被打乱，重新从小到大排序
setInf$grps = grps
knnT = as.data.frame(table(Bayeswh))
setInfF = setInf[order(setInf$grps),]

setInfFB = setInfF

#混淆矩阵
MLB= table(setInfFB$type, Bayeswh, dnn =c("真实值","预测值"))
...
pred = lapply(1:v,function(i, setInf){
  omit = which(grps == i)
  pcl = predict.glm(glm(type ~ OB + DisRwr + goSim + Jaccard + Man,setInf[-omit,],family=Manomial(link="logit")),newdata = setInf[omit,])
},setInf)
#整合测试结果
LGwh = unlist(pred)
#grps 顺序已经被打乱，重新从小到大排序
setInf$grps = grps
knnT = as.data.frame(table(LGwh))
setInfF = setInf[order(setInf$grps),]
...
#混淆矩阵
MLLG= table(setInfFLG$type, LGwh, dnn =c("真实值","预测值"))
MLLG = as.data.frame(MLSVM)


LG_roc = roc(setInfFLG$type, as.numeric(LGwh),levels = c('1', '0'),direction = "<")


plot(LG_roc)
LG_roc$auc
coords(LG_roc, "best")
aupr=AUC(obs=setInfFLG$type,pred=as.numeric(LGwh),curve = "ROC", simplif=TRUE, main = "ROC curve")
aupr=AUC(obs=setInfFLG$type,pred=as.numeric(LGwh),curve = "PR", simplif=TRUE, main = "PR curve")


setInfFLG$LGwh = as.numeric(LGwh)
...
#####smooth####
knn_roc = roc(setInfFKNN$type, as.numeric(KNNwh),levels = c('0', '1'),direction = "<")
SVM_roc = roc(setInfFSVM$type, as.numeric(SVMwh),levels = c('0', '1'),direction = "<")
GBRT_roc = roc(setInfFGBRT$type, as.numeric(GBRTwh),levels = c('0', '1'),direction = "<")
B_roc = roc(setInfFB$type, as.numeric(Bayeswh),levels = c('1', '0'),direction = "<")
LG_roc = roc(setInfFLG$type, as.numeric(LGwh),levels = c('1', '0'),direction = "<")
...
#setInfFKNN$KNNwh = as.numeric(KNNwh)
#write.csv(setInfFKNN,"KNNML.csv",quote = F,row.names = F,fileEncoding = 'GBK')
